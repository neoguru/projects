...
//                    console.log(err);
    ...
    PAGE_SAVE: function (caller, act, data) {
        ...
        if (caller.formView01.validate()) {
            var policyData = caller.formView01.getData();
            var attachList = [].concat(caller.gridView02.getData());
            
            policyData.nmWriter = SCRIPT_SESSION.userNm;
            policyData.noEmployee = SCRIPT_SESSION.noEmployee;
            policyData.attachList = attachList;
            
				console.log(policyData);
           axboot.promise()
           		.then(function (ok, fail, data) {
           			axboot.ajax({
           				type: "PUT", url: "/api/v1/policy", data: JSON.stringify([policyData]),
           				callback: function (res) {
           					ok(res);
           					}
           				});
...
fnObj.uploadView = axboot.viewExtend(axboot.commonView, {
    initView: function () {
    	...
    	upload1.setConfig({
            ...
            form: {
            		action: "/api/v1/policyattach/upload",
            		fileName: "file"
             },
            multiple: true,
//            manualUpload: false,
