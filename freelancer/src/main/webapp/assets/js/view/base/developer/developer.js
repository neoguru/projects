...
//        console.log(data);
		(function(x) {
			if (x == "FREE"){
				...
				$("#formView01").find('[id="panel-free"]').attr("style", "display:none;");
				$("#formView01").find('[id="panel-partner"]').attr("style", "display:block;");
			}
		})(data.typeDeveloper);

        caller.formView01.setData(data);
        caller.formView01.setNmPartner(data.partner.nmPartner);
        if (data.noPartner){
        	caller.formView01.setPartnerData(data.partner);
        	/*        	
            axboot.ajax({
                type: "GET",
                url: "/api/v1/partner",
                data: {noPartner: data.noPartner},
                callback: function (res) {
                    caller.formView01.setPartnerData(res);                    
                  }
              }); 
              */      
        }
...
fnObj.pageStart = function () {
    ...
    axboot.promise()
        .then(function (ok) {
    		...
    		$("#formView01").find('[id="ynJoin"]').attr("disabled", "disabled");
            ACTIONS.dispatch(ACTIONS.PAGE_SEARCH);
        })
        .catch(function () {
...
fnObj.searchView = axboot.viewExtend(axboot.searchView, {
    ...
    getData: function () {
        return {
            ...
            typeDeveloper: this.headType.val(),
            ynJoin: this.headYnJoin.val(),
            headNmDeveloper: this.headNmDeveloper.val(),
            noCompany: SCRIPT_SESSION.noCompany
...
fnObj.gridView01 = axboot.viewExtend(axboot.gridView, {
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                {key: "noDeveloper", label: COL("developer.noDeveloper"), width: 100, align: "center"},
                {key: "nmDeveloper", label: COL("developer.nmDeveloper"), width: 120, align: "center"},
                {key: "typeDeveloper", label: COL("developer.typeDeveloper"), width: 120, align: "center", formatter: function () {                	
                    return parent.COMMON_CODE["DEVELOPER_TYPE"].map[this.value]
            	   }},
             	  {key: "ynJoin", label: COL("developer.ynJoin"), width: 100, align: "center", formatter: function () {                	
...
//            	console.log(returnValue);
    ...
    initEvent: function () {
        ...
        this.model.onChange("typeDeveloper", function () {
            if (this.value == "FREE") {
                _this.target.find('[id="panel-free"]').attr("style", "display:block;");
                _this.target.find('[id="panel-partner"]').attr("style", "display:none;");
            } else {
                _this.target.find('[id="panel-free"]').attr("style", "display:none;");
                _this.target.find('[id="panel-partner"]').attr("style", "display:block;");
            }
        });    
...
//        this.target.find('[data-ax-path="noCustomer"]').attr("readonly", "readonly");
