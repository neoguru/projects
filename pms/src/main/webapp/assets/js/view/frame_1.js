...
fnObj.util = {
    convertList2Tree: function (_list, _childrenKey) {
        ...
        var firstItemLabel = 	' <span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span>' ;
...
//        this.asideHandle = $("#ax-aside-handel");
        ...
        */
//        console.log(this.aside.get(0));
        if (this.aside.get(0)) {
            this.asideView.initView();
            this.asideView.print();
            this.asideView.controlMenu();					/* 2018.02.09 추가 */
        }
    },
                ...
                .on("click", '[data-label-index]', function () {
                    ...
                    if (menuItems[index].children && menuItems[index].children.length) {
                        ...
                        if (menuItems[index].program) {
                            ...
                            $('body').removeClass("show-left");					/* 2018.02.10 추가 */
                            $('body').addClass("hide-left");				/* 2018.02.10 추가 */
                        }
                    }
                });
...
//        	  'use strict';
        ...
        open: function (_index) {
            ...
            if (this.openedIndex != _index) {
				...
				*/
                fnObj.frameView.aside.find('[data-label-index="' + this.openedIndex + '"]');
                fnObj.frameView.aside.find('[data-tree-body-index="' + this.openedIndex + '"]');

                fnObj.frameView.aside.find('[data-label-index="' + _index + '"]');
                fnObj.frameView.aside.find('[data-tree-body-index="' + _index + '"]');
...
fnObj.tabView = axboot.viewExtend({
    ...
    initView: function () {
        this.target = $("#sh-header-tab-container");
